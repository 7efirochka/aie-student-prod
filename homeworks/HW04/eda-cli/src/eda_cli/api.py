...
from .core import compute_quality_flags, missing_table, summarize_dataset, create_json_summary
...
# ----------ВАРИАНТ А: новый HTTP поинт /quality-flags-from-csv: набор флагов качества ----------

@app.post("/quality-flags-from-csv", tags=["new_quality"], summary = "Полный набор флагов")
async def quality_flags_from_csv(file: UploadFile = File(...)):
    ...
    flags_resp: dict[str, bool] = {
        key: bool(value) for key, value in flags_all.items()
    }

    return {"flags": flags_resp}


# ----------ВАРИАНТ В: новый HTTP поинт /summary-from-csv: --json-summary ----------


@app.post("/summary-from-csv", tags = ["new_quality"], summary = "Краткая json сводка по датафрейму")
async def summary_from_csv(file: UploadFile = File(...)):
    ...
    problem_columns = flags_all.get("too_many_missing_list", [])

    return create_json_summary(summary, flags_all, problem_columns)


# ----------ВАРИАНТ С: новый HTTP поинт /sample: вывод рандомных N строк таблицы ----------


@app.post("/sample", tags = ["new_quality"], summary = "Вывод рандомных N строк таблицы")
async def summary_from_csv(file: UploadFile = File(...), n: Optional[int]= 10):
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    try:
        df = pd.read_csv(file.file)
    ...
    if n > len(df):
        raise HTTPException(
            status_code=400,
            detail=f"Запрошено {n} строк, но в датасете только {len(df)} строк"
        )
    else:
        random_rows_in_df = sample(range(len(df)), n)

        row = df.iloc[random_rows_in_df]
        row = row.where(pd.notna(row), None)
        
        for col in row.columns:
                mask_inf = (row[col] == np.inf) | (row[col] == -np.inf)
                row.loc[mask_inf, col] = None
        
        return {"data": row.to_dict("records")}
