...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, max_missing: float = 0.5) -> Dict[str, Any]:
    """
    ...
    flags["max_missing"] = max_missing
    flags["too_many_missing"] = max_missing_share > max_missing

    flags["too_many_missing_list"] = []
    for i in range(len(missing_df)):
        if missing_df["missing_share"].iloc[i] > max_missing:
            col_name = missing_df.iloc[i]
            flags["too_many_missing_list"].append(col_name.name)
    ...
    for col in summary.columns:
        if not col.is_numeric and col.non_null > 0:
            ...
            if unique_ratio > max_unique_ratio:
                ...
                flags["high_cardinality_categoricals"].append({
                    "column": col.name,
...
def create_json_summary(
    ...
    return {
        "dataset_info": {
            "n_roms":summary.n_rows,
            "n_columns": summary.n_cols,
        },

        "quality_score": {
            "quality_score": round(quality_flags["quality_score"], 3),
        },

        "problematic_column": {
            "problematic_column": problem_columns,
        }
